...
#@card = CardLoader.new(1)
#
#[:name, :casting_cost, :converted_casting_cost, :type, :text, :power, :toughness, :expansion, :rarity, :artist, :other_sets, :restrictions, :flavor_text].each do |f|
#  if [:restrictions, :other_sets].include? f
#    $stdout.print "#{f}: "
#    pp @card.send(f)
#  else
#    puts "#{f}: #{@card.send f}"
